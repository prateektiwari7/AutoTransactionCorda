...
@CordaService
class AutoTrigger(private val serviceHub: AppServiceHub) : SingletonSerializeAsToken() {

    private companion object {
        val log = loggerFor<AutoTrigger>()
        ...
        serviceHub.vaultService.trackBy<PaymentRequestState>().updates.subscribe {
            ...
            if (ourIdentity == serviceHub.networkMapCache.getPeerByLegalName(CordaX500Name("BankOperator", "Toronto", "CA"))!!) {
                executor.execute {
                    ...
                    serviceHub.startFlow(PaymentStartedbyService())
                }
            }
        }
        }
